...
class MyModule(nn.Module):
    def __init__(self):
        ...
        self.attention1 = AttentionLayer(16)
        self.attention2 = AttentionLayer(32)
...
class MyModule(nn.Module):
    ...
    def forward(self, x1):
        x1 = self.conv1(x1)
        x1 = self.attention1(x1)  # Apply attention
        x1 = F.relu(self.bn1(x1))
        x1 = self.pooling1(x1)
        x1 = self.conv2(x1)
        x1 = self.attention2(x1)  # Apply attention
        x1 = F.relu(self.bn2(x1))
        x1 = self.pooling2(x1)
        x1 = self.conv3(x1)
        x1 = self.attention3(x1)  # Apply attention
...
def main():
    ...
    (x1_train, y_train, groups_train), (x1_test, y_test, groups_test) = load_data(
        wavelet=wavelet, scales=scales, sampling_rate=sampling_rate)
    print("Data loaded successfully!")

    log_dir = "./logs_attention/{}".format(wavelet)
    ...
    net.save_params(f_params="./models/model_attention_{}.pkl".format(wavelet))
