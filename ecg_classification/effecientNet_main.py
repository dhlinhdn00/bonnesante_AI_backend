...
class CustomEfficientNet(nn.Module):
    def __init__(self, num_classes=4, pretrained=True):
        ...
        self.efficientnet = timm.create_model('efficientnet_b0', pretrained=pretrained, in_chans=1, num_classes=num_classes)
...
def worker(data, wavelet, scales, sampling_period):
    ...
    for i in range(len(r_peaks)):
        ...
        if categories[i] == 4:
            continue
        x1.append(cv2.resize(coeffs[:, r_peaks[i] - before: r_peaks[i] + after], (224, 224)))
...
def main():
    model = CustomEfficientNet(num_classes=4, pretrained=True)
    ...
    # log_dir = "./logs_efficientnet/{}".format(wavelet)
    ...
    # net.save_params(f_params="./models/model_efficientnet_{}.pkl".format(wavelet))
